...
function loadModule(id) {
    ...
    // DEBUGGED AUDIO RESET: Force source clearing to prevent "NotSupportedError"
    audioPlayer.pause();
    audioPlayer.src = ""; 
    audioPlayer.load();
...
function renderContent(mod) {
    ...
    audioToggle.addEventListener('click', () => {
        ...
        // DEBUGGED SOURCE LOGIC: Ensure relative path is clean for GitHub Pages
        if (!audioPlayer.src.endsWith(audioUrl)) {
            audioPlayer.src = audioUrl;
            // Hint to the browser for .m4a files
            audioPlayer.type = "audio/mp4"; 
            audioPlayer.load();
        }
        
        if (audioPlayer.paused) {
            // Audio play() returns a promise, handling it to prevent uncaught errors
